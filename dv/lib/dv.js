...
//takes in a minimum and maximum value. returns a _function_ that outputs a random value between min and max. If min and max are poorly specified, goes to default behavior of randomly generating a value between 0 and 1
...
//returns a _function_ which returns a random integer between a and b
...
//returns a value within a standard deviation of the mean, using the given mean and standard dev
...
dv.type = {
    nominal: "nominal", //labels or categories
    ordinal: "ordinal", //ordinal - ordering (first second third)
    numeric: "numeric",
    unknown: "unknown"
};
...
dv.table = function(input)
{
	...
	//add column to the table. name, values (array), type, isColumn. doesn't return anything
    table.addColumn = function(name, values, type, iscolumn) {
        type = type || dv.type.unknown;

		//compressed is true if the type is nominal or ordinal
        var compress = (type === dv.type.nominal || type === dv.type.ordinal);
        var vals = values;
        
		//if values are nominal or ordinal and iscolumn is true
	...
	//takes in column index and removes it
	...
	//return value stored at indicated index (col,row) of table
...
outer:            
	...
	//returns a table of indices that value f appears
